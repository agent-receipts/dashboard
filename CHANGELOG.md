...
- **Cost column in the grouped Receipts table** — each row now shows the running total spent as of that receipt's own timestamp, sourced from a new `enrich.Enrichment.CostPoints` cumulative-cost curve (sampled after every priced local transcript turn) and looked up per row via `GET /api/sessions/{sessionID}/enrichment`. Local, unverified, and approximate by construction — a receipt has no exact join key to the LLM turn that produced it, so this is "spent so far as of the nearest known local turn," not an exact per-action cost. Flat (non-session-grouped) receipt tables are unchanged.
