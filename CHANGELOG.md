...
- **Transcript-derived model and token usage in receipt detail** — the dashboard now surfaces `issuer.runtime.model`, `issuer.runtime.capture_method`, and token usage (`input_tokens`, `output_tokens`, `cache_read_input_tokens`, `cache_creation_input_tokens`) from `issuer.runtime.usage` when present in a receipt (obsigna PR #779). These fields are shown in the receipt detail modal: `runtime.model` and `runtime.capture_method` appear alongside the existing `runtime.agent_id`/`agent_type` in the Issuer block, and a dedicated **Token usage** row shows the per-call token counts. Older receipts without these fields degrade gracefully. The `runtime_model` field is also included in every `ReceiptRow` API response, and the session delegation graph now prefers the transcript-derived model over the static `issuer.model` when both are present.
