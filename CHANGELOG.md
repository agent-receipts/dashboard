...
- **Local enrichment also shown in the Receipts tab's inline agent graph** — the same condensed tokens/cost line now appears in the "Agent graph" panel shown above the receipts table when filtering by session id, reusing the Session Graph modal's rendering. Unchanged: that panel only appears for sessions with more than one agent.
