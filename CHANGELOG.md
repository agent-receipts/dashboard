...
- **Fleet view** — a new "⬡ Fleet view" button on the Sessions tab opens a full-screen graph of the N most recently-active sessions (default 6) rendered as stacked lanes — one orchestrator-plus-sub-agents cluster per session. **Concurrent cross-session collisions are drawn as magenta dashed edges between lanes**, distinct from same-session state deps (solid blue) and delegation (gray dashed): a collision means two independent agents, possibly under different principals, touched the same global resource *while both sessions were live*. Cross-session overlaps where the sessions ran at different times (`temporal_overlap=false`) are filtered out and reported as a count, so the drawn edges signal genuine contention rather than incidental same-file reuse. Clicking a node highlights its whole state-dependency component — spanning lanes via collision edges — and shows its blast-radius panel. Closes #156.
- **Fleet attribution data layer** — new `FleetAttribution([]sessionID)` and `GET /api/fleet/attribution?limit=N` endpoint compute one combined §4 attribution payload across the N most recently-active sessions (default 6, capped at 12). Agent keys are namespaced `<session_id>::<agent_key>` so each session's orchestrator stays distinct, and state-dependency edges spanning two sessions are flagged `cross_session` — the cross-session collision signal where two independent agents touch the same global resource. Each edge also carries `temporal_overlap`: whether the two sessions were active in overlapping time windows, distinguishing a concurrent collision (genuine contention) from the same resource merely touched at different times. `StatDepEdge` gains `cross_session` and `temporal_overlap`; `NodeAttribution` gains `session_id`; single-session `SessionAttribution` output is unchanged except for the additive `session_id`.
