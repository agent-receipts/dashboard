...
- **Local enrichment also shown in the Receipts tab's inline agent graph** — the same condensed tokens/cost line now appears in the panel shown above the receipts table when filtering by session id, reusing the Session Graph modal's rendering. That panel is no longer hidden for single-agent sessions: it now shows (minus the graph body and its collapse toggle, which have nothing to draw) whenever local enrichment is available, labelled "Session" instead of "Agent graph" when there's no delegation graph to draw.
- **Cost column in the grouped Receipts table** — each row now shows the running total spent as of that receipt's own timestamp, sourced from a new `enrich.Enrichment.CostPoints` cumulative-cost curve (sampled after every priced local transcript turn) and looked up per row via `GET /api/sessions/{sessionID}/enrichment`. Local, unverified, and approximate by construction — a receipt has no exact join key to the LLM turn that produced it, so this is "spent so far as of the nearest known local turn," not an exact per-action cost.
- **Cost column in the Overview recent-receipts table and the Sessions tab** — the Overview mini-table gets the same per-row running-total Cost column as the Receipts tab. The Sessions tab gets a session-total Cost column instead, sourced from `GET /api/sessions`, which now composes each session's enrichment inline (concurrently, one goroutine per session, same pattern as `/api/fleet/signatures`) rather than requiring a client fetch per visible row — that list has no limit param and can be long.

## [0.12.0] - 2026-07-15

### Added

- **Local session enrichment (display-only, unverified)** — the receipt-detail view can now show locally-available agent session data (token usage, context-window %, estimated cost) when the dashboard runs on the same host/user as the agent. The receipt-detail response is now `{ "receipt": <AgentReceipt>, "enrichment": <object|null> }`: enrichment is a **sibling** of the signed receipt, never merged into `credentialSubject` or any signed structure, and always carries `unverified: true` and a `source` naming the agent type (`"claude-code"` to start). The join key is `issuer.session_id`, matched against locally-discoverable session files via a pluggable `enricherSource` registry (name/glob/parse) so a second agent type is additive. Parsed results are cached on `(path, mtime, size)`. Cost comes from a local pricing table keyed by model name, summed per turn at each turn's own model — so a session mixing models (e.g. an Opus main thread with a Haiku subagent) prices each turn correctly; if any turn's model is absent from the table, `estimated_cost_usd` is `null`, never a guess. Subagent (Task-tool) turns, which the agent logs into the same session file, are folded into the token and cost totals and also broken out (`subagent_turns`, `subagent_tokens`, `subagent_cost_usd`); the context-window figure is taken from the last main-thread turn so a trailing subagent turn does not misreport the session's context fill. Zero-usage placeholder lines (Claude Code's `<synthetic>` messages) are ignored. Parsed results are cached per session-file path, validated by mtime and size. A missing, unreadable, malformed, or implausibly large session file yields empty enrichment (`null`), never an error surfaced to the user and never a block on receipt rendering; reads are size-bounded so a hostile or symlinked file cannot force an unbounded allocation. The UI renders enrichment with muted styling, a dashed border, and a "local, unverified" tooltip, visually distinct from signed fields. No changes to the daemon, emitter, or receipt schema. Design rationale: [ADR-0002](docs/adr/0002-local-session-enrichment.md).
...
- **Documented path-reuse limitation in cross-session collision detection** ([#157](https://github.com/agent-receipts/dashboard/issues/157)) — collision edges key on the resource path string as a proxy for logical file identity. A path reused over time for different logical files (one session renames a file into a conventional path, another later writes a different file at that same path) yields a spurious `cross_session` edge. The receipt schema carries no content digest or inode to disambiguate, so this is documented as a known limitation (with a characterization test pinning the behaviour) rather than defended against.
...
- **Forensic key endpoint security model** — `SECURITY.md` now documents the security model of `POST /api/forensic-key` and `POST /api/forensic-key/path`: the guards in place (loopback-only bind, Host-header/DNS-rebinding validation, and cross-origin rejection on both endpoints, plus an `application/json` Content-Type CSRF guard and a `..`/NUL path allowlist on `POST /api/forensic-key/path`), the default home-directory allowlist root, the residual risk of that broad default, and the recommendation to scope `-forensic-key-dirs` to the minimum needed.
...
- **Origin-based CSRF guard on forensic key write endpoints** ([#79](https://github.com/agent-receipts/dashboard/issues/79)) — `POST /api/forensic-key`, `POST /api/forensic-key/path`, and `DELETE /api/forensic-key` now reject requests that carry a cross-origin `Origin` header (host:port mismatch against `r.Host`). A CORS-simple `text/plain` POST from a cross-origin page could previously replace the operator's loaded forensic key or clear it without triggering a preflight; the new guard closes this gap. Requests without an `Origin` header (curl, SDK clients, same-origin navigations) are unaffected.
