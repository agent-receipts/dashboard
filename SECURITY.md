...
| Host-header validation (DNS-rebinding defense) | Every request's `Host` header must name a loopback address, so a remote page cannot resolve its own domain to `127.0.0.1` and drive the loopback-bound server as if it were same-origin. | both endpoints |
